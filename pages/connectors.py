...
Select which identity providers your org uses, enter credentials once,
pull live data, and feed it directly into the audit engine.
"""
...
from connectors import (
    ...
    AWSIAMConnector, SalesforceConnector, ConnectorResult,
)
...
CONNECTORS = {
    ...
    "Google Workspace": {
        ...
        "fields": [
            ...
            {"key": "mock_token",  "label": "Service Account Token (or mock token for testing)",
    ...
    "BambooHR": {
        ...
        "description": "Pull employee records, terminations, and departments from BambooHR. Provides HR Master data.",
        "produces": ["hr_df"],
        "fields": [
    ...
    "Workday": {
        "icon": "📋", "category": "HR System",
        "description": "Pull workers, hires, terminations, and role changes from Workday. Provides HR Master data.",
        "produces": ["hr_df"],
        "fields": [
    ...
    "GitHub": {
        ...
        "description": "Pull org members, teams, and access levels from GitHub. Useful for dev team access reviews.",
        "produces": ["sys_df"],
        "fields": [
    ...
    "Salesforce": {
        "icon": "☁️", "category": "CRM",
        "description": "Pull Salesforce users, profiles, and last login. Useful for CRM access reviews.",
        "produces": ["sys_df"],
        "fields": [
...
def build_connector(name: str, creds: dict):
    """Instantiate the correct connector class from credentials."""
    if name == "Okta":
        return OktaConnector(domain=creds["domain"], api_token=creds["api_token"], fetch_mfa=True)

    if name == "Microsoft Entra ID":
        is_mock = "localhost" in creds.get("tenant_id","") or "mock" in creds.get("tenant_id","")
...
with st.expander("🧪 Testing with Mock Server?", expanded=False):
    st.info(
        ...
        "Then click **Use Mock (Test) Values** on any connector below. "
        "All credentials will be pre-filled and ready to test against your fake Acme Corp organisation."
    )
    st.markdown(f"Mock server health check: `http://localhost:5000/health`")
    ...
    for ci, conn_name in enumerate(connector_names):
        ...
        with col:
            with st.container(border=True):
                ...
                tags = []
                if "hr_df"  in produces: tags.append("✅ Provides HR data")
                if "sys_df" in produces: tags.append("✅ Provides System Access data")
                ...
                for field in cfg["fields"]:
                    ...
                    input_type = "password" if field["secret"] else "default"
                    val        = st.text_input(
                        field["label"],
                ...
                if b1.button("🧪 Use Mock Values", key=f"mock_{conn_name}", use_container_width=True):
                    for field in cfg["fields"]:
                        mock_val = cfg["mock_values"].get(field["key"], "")
                        st.session_state[f"conn_{conn_name}_{field['key']}"] = mock_val
                    st.rerun()

                if b2.button("🔗 Test Connection", key=f"test_{conn_name}", use_container_width=True):
                ...
                if b3.button("⬇️ Pull Data", key=f"pull_{conn_name}", use_container_width=True,
                             type="primary"):
                    filled = {f["key"]: st.session_state.get(f"conn_{conn_name}_{f['key']}","")
                              for f in cfg["fields"]}
                    if not all(filled.values()):
                        ...
                        with st.spinner(f"Fetching from {conn_name}…"):
                            try:
                                ...
                                if result.success:
                                    st.success(f"✅ {result.user_count} records pulled from {conn_name}")
                                else:
...
# ─────────────────────────────────────────────────────────────────────────────
#  COMBINE & SEND TO AUDIT ENGINE
# ─────────────────────────────────────────────────────────────────────────────

st.markdown("### 🚀 Send to Audit Engine")

results = st.session_state.get("connector_results", {})
pulled  = {k: v for k, v in results.items() if v.success}

if not pulled:
    st.info("Pull data from at least one connector above, then click Send to Audit Engine.")
else:
    st.markdown(f"**{len(pulled)} connector(s) with data ready:**")
    for name, r in pulled.items():
        cfg = CONNECTORS[name]
        st.markdown(f"- {cfg['icon']} **{name}** — {r.user_count} records "
                    f"({'HR + System' if not r.hr_df.empty and not r.sys_df.empty else 'HR only' if not r.hr_df.empty else 'System only'})")

    if st.button("🚀 Merge All & Send to Audit Engine", type="primary", use_container_width=True):
        all_hr  = [r.hr_df  for r in pulled.values() if not r.hr_df.empty]
        all_sys = [r.sys_df for r in pulled.values() if not r.sys_df.empty]

        combined_hr  = pd.concat(all_hr,  ignore_index=True).drop_duplicates(subset="Email", keep="first") if all_hr  else pd.DataFrame()
        combined_sys = pd.concat(all_sys, ignore_index=True) if all_sys else pd.DataFrame()
        ...
        st.success(
            f"✅ Data merged — {len(combined_hr)} HR records · {len(combined_sys)} system access records. "
            f"Go to the **Tool** page and the audit will use this live data automatically."
        )

        if not combined_hr.empty:
            st.markdown("**HR data preview (first 5 rows):**")
            st.dataframe(combined_hr.head(5), use_container_width=True, hide_index=True)
        if not combined_sys.empty:
            st.markdown("**System access preview (first 5 rows):**")
